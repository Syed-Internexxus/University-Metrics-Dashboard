...
st.set_page_config(
    "Career Outcomes Analytics", "🎓", layout="wide", initial_sidebar_state="expanded"
)
...
def load_df() -> pd.DataFrame:
    ...
    df["YearQuarter"] = (
        df["RegisteredDate"].dt.year.astype(str) + " Q" + df["Quarter"].astype(str)
    )
    return df
    ...
    fig = go.Figure(
        go.Indicator(
            ...
            gauge=dict(
                ...
                steps=[
                    dict(range=[0, thr[0]], color=hex_to_rgba(colors[0], 0.3)),
                    dict(range=[thr[0], thr[1]], color=hex_to_rgba(colors[1], 0.3)),
                    dict(range=[thr[1], 100], color=hex_to_rgba(colors[2], 0.3)),
                ],
            ),
...
def create_area_chart(data, x, y, title, color=CLR_PRIMARY):
    ...
    fig.update_traces(
        line=dict(width=2, color=color), fillcolor=hex_to_rgba(color, 0.2)
    )
    fig.update_layout(
        title=title,
        height=130,
        ...
        yaxis_showgrid=True,
        yaxis_gridcolor=hex_to_rgba(CLR_SECONDARY, 0.2),
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        font_color=CLR_TEXT,
    )
...
# ────── SIDEBAR ────────────────────────────────────────────────
st.markdown(
    "<h1 class='dashboard-title'>Career Outcomes Analytics</h1>", unsafe_allow_html=True
)
st.markdown(
    "Interactive platform for tracking student career journey "
...
with st.sidebar:
    ...
    with st.expander("How to Use"):
        st.markdown(
            "1. **Filter** by major and graduation year  \n"
            "2. **Click** charts  \n3. **Hover** elements  \n"
            "4. **Download** insights"
        )
...
with c1:
    ...
    st.plotly_chart(
        create_bar_chart(
            quarterly, "YearQuarter", "Students", "Quarterly Registrations"
        ),
...
with c2:
    ...
    st.plotly_chart(
        create_gauge_chart(rate, "Overall Rate", [60, 80]), use_container_width=True
    )
    ...
    st.markdown(
        f"<p class='caption'><span class='{css}'>{change:+.1f}%</span> "
        f"vs previous year</p></div>",
        unsafe_allow_html=True,
    )

with c3:
    med_gap = (
        int(df_f["DaysToFullTimeJob"].dropna().median())
        if not df_f["DaysToFullTimeJob"].dropna().empty
        else 0
    )
    st.markdown("<div class='card'>", unsafe_allow_html=True)
    ...
    hist = px.histogram(
        ...
        nbins=15,
        opacity=0.7,
        color_discrete_sequence=[hex_to_rgba(CLR_WARNING, 0.7)],
    )
    if len(dist) > 0:
    ...
    st.markdown(
        "<p class='caption'>Distribution (days after graduation)" "</p></div>",
        unsafe_allow_html=True,
    )
...
with c4:
    ...
    st.plotly_chart(
        create_gauge_chart(ipa, "Interviews per 100 Applications", [10, 20], suffix=""),
        use_container_width=True,
    )
    st.markdown(
        f"<p class='caption'>Avg {df_f['ApplicationsSubmitted'].mean():.1f} "
        f"applications per student</p></div>",
        unsafe_allow_html=True,
    )

# ────── PIPELINE & MAJOR PERFORMANCE ───────────────────────────
st.markdown(
    "<div class='section-header'>Pipeline Analytics</div>", unsafe_allow_html=True
)
    ...
    funnel = go.Figure(
        go.Funnel(
            ...
            marker=dict(
                color=[
                    CLR_PRIMARY,
                    hex_to_rgba(CLR_PRIMARY, 0.9),
                    hex_to_rgba(CLR_PRIMARY, 0.8),
                    hex_to_rgba(CLR_PRIMARY, 0.7),
                    hex_to_rgba(CLR_PRIMARY, 0.6),
                    CLR_SUCCESS,
                ]
            ),
...
with g2:
    ...
    with tab2:
        ...
        line = px.line(
            time_data, x="GraduationYear", y="Placement", color="Major", markers=True
        )
        line.update_layout(
            height=360,
            margin=dict(l=10, r=10, t=10, b=10),
        ...
        line.update_xaxes(
            tickfont=dict(color=CLR_TEXT), title_font=dict(color=CLR_TEXT)
        )
        line.update_yaxes(
            tickfont=dict(color=CLR_TEXT), title_font=dict(color=CLR_TEXT)
        )
...
# ────── DETAILED INSIGHTS ───────────────────────────────────────
st.markdown(
    "<div class='section-header'>Detailed Insights</div>", unsafe_allow_html=True
)
...
with b1:
    ...
    with sub1:
        ...
        st.markdown(
            "<p class='caption'>Distribution of time‑to‑employment "
            "by major. Lower is better.</p>",
            unsafe_allow_html=True,
        )

    with sub2:
        uni = (
            df_f.groupby("University")
            .agg(
                Students=("StudentID", "count"),
            ...
            .assign(
                Placement=lambda d: (d["Placement"] * 100).round(1),
        ...
        tbl = go.Figure(
            go.Table(
                header=dict(
                    values=["University", "Students", "Placement %", "Days to Job"],
                    fill_color=CLR_CARD,
                    align="left",
...
with b2:
    ...
    with r1:
        ...
        sc.add_annotation(
            x=xs.max() * 0.8,
            y=df_f["InterviewInvites"].max() * 0.9,
            text=f"Each workshop increases<br>interview chances " f"by {m:.2f}×",
            showarrow=True,
            arrowhead=2,
            arrowcolor=CLR_TEXT_SECONDARY,
            arrowsize=1,
            arrowwidth=1.5,
            bordercolor=CLR_CARD,
            borderwidth=1,
        ...
        )  # Ensure annotation text is black
        sc.update_layout(
            height=400,
            margin=dict(l=10, r=10, t=10, b=10),
        ...
        st.markdown(
            "<p class='caption'>Students who attend more workshops "
    ...
    with r2:
        if "StartingSalary" in df_f.columns:
            ...
            st.markdown(
                "<p class='caption'>Salary distribution by major shows "
...
st.markdown(
    "<div class='section-header'>Placement Outcomes</div>", unsafe_allow_html=True
)
                ...
                with tabs[i]:
                    ...
                    with r:
                        stats = {
                            ...
                            m: [
                                d["ApplicationsSubmitted"].mean()
                                / df_f["ApplicationsSubmitted"].mean()
                                * 100,
                                d["InterviewInvites"].mean()
                                / df_f["InterviewInvites"].mean()
                                * 100,
                                d["WorkshopAttendance"].mean()
                                / df_f["WorkshopAttendance"].mean()
                                * 100,
                                d["InternshipPlacement"].mean()
                                / df_f["InternshipPlacement"].mean()
                                * 100,
                                d["FullTimePlacement"].mean()
                                / df_f["FullTimePlacement"].mean()
                                * 100,
                            ],
                            "Average": [100] * 5,
                        }
                        ...
                        rp.update_traces(
                            fill="toself", fillcolor=hex_to_rgba(CLR_PRIMARY, 0.25)
                        )
                        rp.add_trace(
                            go.Scatterpolar(
                                r=[100] * 5,
                                ...
                                fill="toself",
                                name="Average",
                                fillcolor=hex_to_rgba(CLR_SECONDARY, 0.125),
                        ...
                        st.caption(
                            "Performance relative to cohort average "
...
st.markdown(
    f"Career Outcomes Analytics Dashboard | "
    f"Last updated: {datetime.now():%B %d, %Y}",
    unsafe_allow_html=True,
)
st.markdown("</div>", unsafe_allow_html=True)
