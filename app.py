...
# ------------------------------------------------------------------
#  Streamlit dashboard – colour‑matched to Internexxus hero section
# ------------------------------------------------------------------
...
# ───── Palette taken from the screenshot ─────────────────────────
CLR_MINT       = "#16D5A8"   # primary accent (mint)
CLR_SKY_A      = "#C6E5F6"   # light sky
CLR_SKY_B      = "#C5EBF0"   # aqua tint
CLR_SKY_C      = "#D9F8F0"   # very light mint
CLR_SKY_D      = "#BCD9FB"   # lavender/sky
CLR_TEXT       = "#1F2B46"   # default text
CLR_CARD_BG    = "#FFFFFF"   # white tiles
...
st.markdown(
    ...
    html, body, [class^="css"] {{
        ...
        color: {CLR_TEXT};
        /* Hero‑like radial gradient background */
    ...
    .card h3 {{ font-size:1rem; margin:0 0 .25rem 0; color:{CLR_MINT}; letter-spacing:.3px; }}
...
with st.sidebar:
    st.header("Filters")
    majors_f = st.multiselect(
        "Major", sorted(df["Major"].unique()), df["Major"].unique()
    )
    years_f = st.multiselect(
        "Grad Year",
...
with c2:
    ...
    st.markdown(
        "<div class='card' title='Percent with FT offers'>",
        unsafe_allow_html=True,
    )
    ...
    st.plotly_chart(
        spark(rolling, "FullTimePlacement", CLR_SKY_D), use_container_width=True
    )
    st.markdown(
...
with c3:
    ...
    st.markdown(
        "<div class='card' title='Median days to FT job'>",
        unsafe_allow_html=True,
    )
    ...
    st.plotly_chart(
        spark(gap_hist, "cnt", "#FFA629"), use_container_width=True
    )
    st.markdown(
...
with c4:
    ...
    st.markdown(
        "<div class='card' title='Average applications submitted'>",
        unsafe_allow_html=True,
    )
    ...
    st.plotly_chart(
        spark(by_major, "avg", CLR_SKY_B), use_container_width=True
    )
    st.markdown(
...
with g1:
    ...
    funnel = px.area(
        x=list(stages), y=list(stages.values()), template="plotly_white"
    )
...
with g3:
    ...
    uni_tbl = go.Figure(
        go.Table(
            ...
            cells=dict(
                ...
                fill_color=[CLR_CARD_BG] * len(uni.columns),
                font=dict(color=CLR_TEXT, family="Poppins"),
                align="left",
            ),
        )
    ...
    with st.expander("ℹ️ Why this matters"):
        st.write(
            "High FT placement signals strong employer ties & student readiness."
        )
...
# ── Footer ─────────────────────────────────────────────────────
st.markdown(
    "<div style='text-align:center;margin-top:25px;color:#6C7DA0'>© 2025 University Career Insights Dashboard</div>",
    unsafe_allow_html=True,
)
